...
- P9b 大型 report / NDJSON 輔助輸出已完成第一版；GUI log artifacts 會新增 `checked.ndjson`、`broken.ndjson`、`external-links.ndjson`，GUI SSE complete event 不再傳完整 report，Analyzer 具備大檔提示、載入狀態、列表「載入更多」，並可匯入 `broken.ndjson` / `external-links.ndjson`。P9c-1 Rules Schema 與 P9c-2 Rules 追溯 / rules URL 載入安全化已完成第一版。
