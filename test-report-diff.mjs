...
function assertSummarySkeleton(diff, schema) {
  ...
  for (const key of requiredSummary) {
    ...
    assert(diff.summary[key] === 0, `P6.3 skeleton summary field should still be 0: ${key}`);
  }
}
...
function assertP63Skeleton(diff) {
  assert(diff.urlChanges.length === 0, "P6.3 should not emit urlChanges yet.");
  assert(diff.externalChanges.length === 0, "P6.3 should not emit externalChanges yet.");
  assert(diff.diagnosticsChanges.length === 0, "P6.3 should not emit diagnosticsChanges yet.");
}
