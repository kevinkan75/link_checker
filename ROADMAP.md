...
P6.5a 已完成輸出契約基線、redaction、sources 上限、response body limit 與 **Header / Accept-Encoding / Keep-Alive**。P6.5b-1 SSRF / URL security policy 已完成；下一個主線收斂為 P6.5b-2 `runStatus` / partial report。Stage 0 若再發現 GUI/CLI 落差，可穿插小修，但不得改掃描語意或既有 report 主契約。
...
| 4 | P6.5b | 進行中 | SSRF 已完成；後續 partial report、robots / compliance、Retry-After、WAF schema | WAF/Bot 繞過 |
...
狀態：進行中；P6.5b-1 已完成。實作前評估見 [docs/P6_5B_ASSESSMENT.md](docs/P6_5B_ASSESSMENT.md)。此階段會新增 report 語意、掃描安全邊界與合規紀錄，需同步更新 CLI、GUI 顯示與 Analyzer fallback。
...
| 類別 | 細項 | 落點 |
| --- | --- | --- |
| schema version | `1.0.0` 舊 report、`1.1.0` 新增 `schemaVersion` / `generator` / redaction / body limit / `sourcesTruncated`、`1.2.0` 新增 `securityPolicy`，後續擴充 robots / compliance / protection 結構 | P6.5a / P6.5b |
