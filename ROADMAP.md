...
| P13-6 Response Body Timeout Lifecycle | DONE | Deterministic fixture 已確認 native fetch timeout 原先在 headers 到達時結束，未涵蓋 required response-body handling。Bounded correction 將既有 AbortController / response cleanup lifecycle 延伸至 body 完成或取消，保留 diagnostic-body timeout 已取得的正式 HTTP evidence，並使 `elapsedMs` 對齊實際 response handling。 |
