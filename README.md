...
Local Link Checker 是可在本機執行的無效連結檢測輔助工具。它會從指定網址開始檢查同網域頁面、盤點外部連結，並輸出問題連結清單、摘要與完整報告。
...
工具回報的「問題連結」不一定都代表連結已失效。部分結果只是需要人工確認。

| 類型 | 代表意義 | 建議處理 |
| --- | --- | --- |
| `404 / 410` | 頁面或資源不存在 | 優先確認並修正或移除連結 |
| `403` | 存取被拒 | 確認是否需要登入、權限、Cookie、Referer 或特定 User-Agent |
| `429` | 請求過快或被限流 | 降低檢查速度後重試 |
| 防護阻擋 | WAF/Bot/CDN 拒絕程式化請求 | 用瀏覽器人工確認，或與網站管理方協調 |
| timeout / 網路錯誤 | 網路、DNS、TLS 或連線問題 | 重試，或確認網址與網路環境 |
| redirect | 網址會轉到其他位置 | 若可正常開啟，不一定需要修正 |

用語建議：

- 問題連結：廣義問題，包含 `404`、`410`、`403`、timeout、防護阻擋與網路錯誤。
