...
先看 GUI 或 `broken.csv` 中的「建議處理」與「是否需人工確認」。對多數交辦情境，可以先分成三類：

| 使用者判讀 | 常見畫面或報告訊號 | 建議處理 |
| --- | --- | --- |
| 明確需要處理 | 確認不存在、轉址後失敗、轉址循環、轉址次數過多 | 修正、移除或更新來源頁連結。 |
| 需要人工確認 | 存取被拒（HTTP 403）、限流（HTTP 429）、連線逾時、網路錯誤、防護阻擋、TLS 憑證問題 | 用一般瀏覽器確認；必要時請網站管理方或外站窗口協助判讀。 |
| 正常／目前可用 | 可開啟、轉址後仍可開啟、頁面品質提醒 | 通常不需立即修正；轉址與品質提醒可視維護需求排入例行處理。 |

報告中仍會保留技術分類，例如 `action_required`、`needs_review`、`external_limited`、`redirect_ok` 與 `ok`。一般使用者不需要先理解這些代碼；需要追查欄位定義時再看 [docs/TECHNICAL_SPEC.md](docs/TECHNICAL_SPEC.md)。
...
工具會對部分同站 `404 / 410` 再確認一次，目的是降低暫時性錯誤造成的誤判。常見結果：
